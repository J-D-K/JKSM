...
namespace
{
    // This is the map where strings are mapped according to name and index. None of these are going to be anywhere close to 0xFF.
...
inline std::string GetFilePath(void)
{
    ...
    if (!TestFile)
    {
        return std::string("romfs:/Strings/" + std::string(s_FileMap[CFG_LANGUAGE_EN].data()));
    }
