...
namespace
{
    ...
    // The system language. I decided to put it here since it's part of CFGU. I also don't think calling it over and over is a
    // good idea.
...
{
    if (s_ConfigMap.find(Key.data()) == s_ConfigMap.end()) { return -1; }
    return s_ConfigMap.at(Key.data());
}

void Config::SetByKey(std::string_view Key, uint8_t Value)
{
    if (s_ConfigMap.find(Key.data()) == s_ConfigMap.end()) { return; }
