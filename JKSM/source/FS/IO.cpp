...
namespace
{
    ...
    constexpr size_t FILE_BUFFER_SIZE = 0x20000;
...
void FS::CopyDirectoryToDirectory(System::ProgressTask *Task, const FsLib::Path &Source, const FsLib::Path &Destination, bool Commit)
{
    ...
    for (uint32_t i = 0; i < SourceDir.GetEntryCount(); i++)
    {
        ...
        {
            // Full paths.
            FsLib::Path FullSource = Source / SourceDir.GetEntryAt(i);
            FsLib::Path FullDestination = Destination / SourceDir.GetEntryAt(i);
...
void FS::CopyDirectoryToZip(System::ProgressTask *Task, const FsLib::Path &Source, zipFile Destination)
{
    ...
    for (uint32_t i = 0; i < SourceDir.GetEntryCount(); i++)
    {
        ...
        else
        {
            ...
            // Full path.
            FsLib::Path ZipFileName = Source / SourceDir.GetEntryAt(i);
            ...
            int ZipError =
                zipOpenNewFileInZip64(Destination, UTF8Buffer, &FileInfo, NULL, 0, NULL, 0, NULL, Z_DEFLATED, Z_DEFAULT_COMPRESSION, 0);
            if (ZipError != Z_OK)
            {
